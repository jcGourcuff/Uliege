...
from nilm.appliance_tracker_V3 import Tracker
...
sampled = df.resample('1min').mean().fillna(method='ffill')[
    ['refrigerator', 'lighting', 'dishwasher']]
sampled.head()
...
# Instanciate a tracker and a list to hold the results, fits the sacler.
T = Tracker(aggregated=filtered[0], T_detect=60, T_error=.1, train_window=15)
T.fit_scaler(data=np.concatenate([f.to_numpy() for f in filtered]))
results = []
