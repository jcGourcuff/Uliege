...
class DataSets():
    ...
    def REDD(house_nb, demo=False):
        ...
        def mydateparser(x): return datetime.fromtimestamp(int(x))

        if demo:
            assert house_nb == 1
            file_loc = file_loc = "data/REDD_demo/low_freq/house_" + \
                str(house_nb) + "/"
        ...
        for id, name in zip(labels['id'], labels['name']):
            if 'mains' not in name:
                ...
                dfs.append(pd.read_csv(loc, sep=" ", names=["timestamp", name],
                                       parse_dates=['timestamp'], date_parser=mydateparser).set_index('timestamp'))
    ...
    def BCR(phase_nb):
        ...
        """
        file_loc = "data/BCR/Load_L" + str(phase_nb) + \
            ".zip"  # I store zipped csvs to save space.

        def mydateparser(x): return datetime.strptime(x, "%d/%m/%Y %H:%M")
