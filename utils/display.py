...
class Graphs:
    ...
    def decompose_aggregate(data_set, unit='watt', title="Appliances contribution to the aggregated power"):
        ...
        aggregated = data_set.sum(axis=1)
        fig = plt.figure(figsize=FIG_DIM)
        list_app = [(name, col) for name,col in zip(data_set.columns, pool)]
        for name,col in list_app[::-1]:
            next = aggregated - data_set[name]
            plt.fill_between(aggregated.index, next, aggregated, color=col, label=name)
            aggregated = next
        plt.ylabel(unit)
        plt.title(title)
        fig.autofmt_xdate()
