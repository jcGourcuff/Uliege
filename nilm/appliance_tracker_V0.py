...
class Tracker():
    ...
    def next_step(self, y, pass_time=True):
        """
        ...
        if dy > self.T_detect:
            ...
            if error > self.T_error:
                ...
                self.next_step(y, pass_time=False)
    ...
    def KP_greedy(self, y):
        ...
        X = pd.Series([a.power_level for a in self.dictionary],
                      dtype='float64').sort_values()
        w_conso = 0
    ...
    def improved_bellman(self, y, tol):
        ...
        weights = np.array([(k, a.power_level) for k, a in enumerate(
        ...
        if len(weights) == 0:
            ...
            for k in range(len(weights)):
                R_prim = [x + weights[k][1]
                          for x in R if x + weights[k][1] < c]
